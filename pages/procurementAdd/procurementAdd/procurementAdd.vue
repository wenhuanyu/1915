            ...
            <view class="container info mb-20">
				...
				<view class="group">
				    ...
				    <view class="outModal" @click="openModal('client')">
				        <uni-easyinput class="input" v-model="clientName" placeholder="请选择供应商"
				            placeholder-style="color:#8D8D8D;" disabled>
				        </uni-easyinput>
				    </view>
				</view>
            ...
            <view class="container good ">
                <view class="add_list" v-for="(item,index) in list" :key="item.id">
					...
					<view class="group">
					   ...
					   <uni-easyinput class="input" v-model="item.remark" placeholder="输入备注内容">
					   </uni-easyinput>
					</view>
        ...
        methods: {
            ...
            submit() {
				...
				this.list.map((item,index) =>{
					a.push({'id':item.id,'num':item.num,'remark':item.remark})
				})
				uni.showModal({
					title: '提示',
					content: '确定当前操作吗？',
					success: res => {
						if(res.confirm) { 
							this.http({
							    url: this.api.warehouse_purchase,
							    method: "POST",
							    data: {
							        uid: this.uid,
									warehouse_id:this.WarehouseId,
									apply_id:this.clientId,
