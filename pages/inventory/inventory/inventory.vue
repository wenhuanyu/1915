<template>
	<view class="background fix-padding">
		...
		<view class="container add">
			<navigator url="/pages/inventory/pandianzhong/pandianzhong">
		    ...
		    <checkbox-group name="check" @change="changeCheck" class="check">
				<view class="item" v-for="(item,index) in list" :key="item.id">
				    ...
				    <view class="flex top .mb-10">
				        <view class="title grey">盘点数量</view>
				        <view class="value">{{item.num}}</view>
				    </view>
				    <view class="flex .mb-10">
				        <view class="title grey">盘点时间</view>
		...
		<view class="container fix-bottom flex_between">
		    ...
		    <button class="btn-search" @click="del">撤销</button>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			detail(eve){
				uni.navigateTo({
					url:'/pages/inventory/details/details?data='+encodeURIComponent(JSON.stringify(eve))
				})
			}
