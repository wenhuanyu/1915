<template>
	<view class="background">
		...
		<view class="container fix-bottom flex" v-if="list[0].check_status == 0">
			<button class="btn-submit" style="width: 100%;" @click="submit_shenhe" >审核通过</button>
			<button class="btn-submit" style="width: 100%;background:#F96164 ;margin-left: 20rpx;" @click="submit_jujue">审核拒绝</button>
		</view>
	</view>
</template>
			...
			submit_shenhe() {
				uni.showModal({
					...
					success: res => {
						if(res.confirm) { 
							this.http({
							    url: this.api.warehouse_purchase_check,
							    method: "POST",
							    data: {
