    ...
    <view class="fix-padding background" @click="hideIsShow">
        <view :class="preventEvent ? 'prevent': ''">
            <view class="container info mb-20">
				...
				<view class="group">
				    <view class="title">供货商名</view>
				    <view class="outModal" @click="openModal('client')">
				        <uni-easyinput class="input" v-model="clientName" placeholder="请选择供货商"
				            placeholder-style="color:#8D8D8D;" disabled>
				        </uni-easyinput>
				    </view>
				</view>
            ...
            <view class="container good ">
                <view class="add_list" v-for="(item,index) in list" :key="item.id">
					...
					<view class="group">
					   ...
					   <uni-easyinput type="number" class="input" :value="item.num" @input="num($event,index)" placeholder="请输入"
					       placeholder-style="color:#8D8D8D;" >
					   </uni-easyinput>
					</view>
					<view class="group">
					    <view class="title">商品单价</view>
					   <uni-easyinput type="number" class="input" :value="item.price" @input="price($event,index)" placeholder="请输入"
					   ...
					   <view v-if="price_show" class="priceList">
							<view v-for="(i,e) in price_list" :key="e"  @tap="priceChange(i,e)" class="price_item" >
								<view>{{i.price}}</view>
							</view>
					   </view>
					</view>
    ...
    export default {
        ...
        methods: {
            ...
            selectClient(i) {
				if(this.kehu_num == 1) {
					...
					this.list.map(item =>{
						...
						item.gonghuo_id = this.clientList[i].id
						item.gonghuo = this.clientList[i].nick_name
					...
					this.list[this.itemIndex].gonghuo_id = this.clientList[i].id
					this.list[this.itemIndex].gonghuo = this.clientList[i].nick_name
            ...
            selectGood(i,e) {
                ...
                this.http({
                    ...
                    success: res => {
						...
						this.list[this.itemIndex].gonghuo =  this.clientName
						this.list[this.itemIndex].gonghuo_id = this.clientId
			...
			assignment() {
				...
				that.list[that.list.length-1].gonghuo_id = that.clientId
				that.list[that.list.length-1].gonghuo =  that.clientName
			},
			// 扫一扫完成按钮
			popups_submit() {
